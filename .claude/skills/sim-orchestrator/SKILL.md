...
- Wire sims into Onli Synth:
  ...
  - Update vite.config.ts to proxy /api/v1 → port 4001, /marketplace/v1 → port 4002
...
4. Start sims: `node packages/marketsb-sim/src/index.ts` → port 4001
5. `curl http://localhost:4001/api/v1/virtual-accounts?ownerRef=user-001` → BalanceDTO[]
6. Start species sim: `node packages/species-sim/src/index.ts` → port 4002
7. `curl -X POST http://localhost:4002/marketplace/v1/eventRequest -H "Content-Type: application/json" -d '{"eventId":"test","intent":"buy","quantity":100}'` → 202
